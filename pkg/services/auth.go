...
func (a *apiService) AuthSession(ctx context.Context, params api.AuthSessionParams) (*api.SessionHeaders, error) {

	if !params.AuthToken.IsSet() {
		return nil, nil
	}

	claims, err := auth.VerifyUser(a.db, a.cache, a.cnf.JWT.Secret, params.AuthToken.Value)

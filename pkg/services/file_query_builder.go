...
func (afb *fileQueryBuilder) execute(filesQuery *api.FilesListParams, userId int64) (*api.FileList, error) {
	...
	return &api.FileList{Items: files,
		Meta: api.FileListMeta{Count: api.NewOptInt(count),
			TotalPages:  api.NewOptInt(int(math.Ceil(float64(count) / float64(filesQuery.Limit.Value)))),
			CurrentPage: api.NewOptInt(filesQuery.Page.Value)}}, nil
}
